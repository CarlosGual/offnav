...
from offnav.dataset.dataset import MetaEpisodeIterator
...
class MetaEnv:
    ...
    def __init__(
        ...
        if self._dataset:
            ...
            # Set up random task for init
            self._episode_iterator.set_task(self.episodes[0].goals_key)
    ...
    def _setup_episode_iterator(self):
        ...
        iter_option_dict = {
            k.lower(): v
            for k, v in self._config.META.METAITERATOR.items()
        }
    ...
    def set_task(self, task_id):
        ...
        self._episode_iterator.set_task(task_id)
    ...
    def _update_step_stats(self) -> None:
        ...
        if self.episode_iterator is not None and isinstance(
            self.episode_iterator, MetaEpisodeIterator
        ):
...
class MetaRLEnv(gym.Env):
    ...
    def set_task(self, task_id) -> None:
        ...
        self._env.set_task(task_id)

    @property
    def task_id(self) -> str:
        return self._task_id

    @profiling_wrapper.RangeContext("MetaRLEnv.reset")
    def reset(self) -> Observations:
        assert self.task_id is not None, "Task id is not set. Use set_task() to set the task id before reset call."
...
@habitat.registry.register_env(name="SimpleMetaRLEnv")
class SimpleMetaRLEnv(MetaRLEnv):
    ...
    def sample_tasks(self, num_tasks):
        assert num_tasks <= len(self._available_tasks), ("num_tasks should be less than or equal to the number of "
                                                         "available goals")
        return [task for task in random.sample(self._available_tasks, num_tasks)]
