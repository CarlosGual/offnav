...
from offnav.policy.models.resnet_gn import ResNet


def write_dataset(data_writer, rollout):
    ...
    for k in list(observations):
        ...
        observations[k] = torch.squeeze(torch.flatten(observations[k].cpu(), start_dim=0, end_dim=1)).detach().numpy()

    # Flatten the dimension associated with the parallelism in habitat
    actions = torch.squeeze(torch.flatten(rollout.buffers['actions'].cpu(), start_dim=0, end_dim=1)).detach().numpy()
    rewards = torch.squeeze(torch.flatten(rollout.buffers['rewards'].cpu(), start_dim=0, end_dim=1)).detach().numpy()
    dones = torch.squeeze(torch.flatten(rollout.buffers['masks'].cpu(), start_dim=0, end_dim=1)).detach().numpy()
    hidden_state = torch.squeeze(rollout.recurrent_hidden_states.cpu()).detach().numpy()

    # Append data to data writer
